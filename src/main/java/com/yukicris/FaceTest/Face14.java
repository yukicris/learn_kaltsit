...
public class Face14 {
    ...
    //线程池未完成服务炸了怎么处理
    // --提供一个思路,在线程提交之前对执行的线程增加一个类似数据库的版号信息,对其进行保存,然后在服务器恢复正常时候对其进行修复
    //--如果队列里的任务请求只存在于内存，没有持久化的话，服务宕机队列中的请求肯定会宕机。可以在提交到线程池前，先把任务信息插到数据库，
    //--状态为未提交，提交到线程池之后改为已提交，任务完成之后改为已完成。服务器宕机后通过后台线程把未提交和已提交的任务重新加入到线程池中(比如服务恢复时自启检测)。
...
————————————————
    ...
    //顺便回忆下码位转换  16--》10——》2                 2进制是   2^0+2^1+2^2+....         比如277 转2进制,可以先转16,160是1011   277=16*17+5 17大于16进位+16*1 10000+5即100010101
