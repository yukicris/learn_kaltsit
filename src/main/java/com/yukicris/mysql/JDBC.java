...
public class JDBC {
     ...
     *
     * 读已提交(Read Committed)：禁止脏读，但允许不可重复读(一个事务不能读取另一个事务已经提交的数据。),一个事务要等另一个事务提交后才能读取数据。
     *
     * 可重复读(Repeatable Read)：禁止脏读和不可重复读，但允许幻读(即一个事务不能读取另一个事务插入的数据),在开始读取数据（事务开启）时，不再允许修改操作。
     *
     ...
     *  直接查name bir，不行，name后的都会失效，中间兄弟不能断
     *  1 最左前缀的原则   2 mysql引擎在查询中未来更好利用索引,在查询过程中会动态调整字段顺序以便利用索引
     *
     *  带头大哥不能死，中间兄都不能断(左前缀原则)，索引列上无操作(不能添加函数,计算,类型转换等)，范围右边全失效(查询范围值,比如age>25会失效,匹配到等于就停止了)，like百分加右边(以%开头查询like会失效)，字符串中有引号(不加引号会让mysql底层做类型转换加上引号,导致与列上无操作冲突)，索引加or
